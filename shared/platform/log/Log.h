...
namespace ks::log
{
    ...
    public:
        ...
        const GUID kGuid; // Immutable event identity for tracking related rows.
    };
    ...
    struct Event
    {
        // recordSequence: Monotonically increasing unique sequence number assigned by the log manager during archival; used only for UI incremental updates, not displayed to users.
    ...
    private:
        mutable std::mutex mutex_;   // Protects m_events and m_revision.
        std::vector<Event> events_;  // Complete archived log stream.
        std::size_t revision_ = 0;   // Incremented after add/clear.
        std::uint64_t nextRecordSequence_ = 1; // Unique sequence number for the next archived log; not reset after clear to avoid UI key collisions.
    };
    ...
    private:
        ...
        const Level kMLevel; // Severity fixed at stream construction.
    };
