...
Fl_Group* createContainerPage(int x, int y, int w, int h) {
    ...
    const int kRightX = x + w / 2 + 8; const int kRightW = w / 2 - 22; const int kInspectorH = std::max(136, h - 226 - 118 - kGap);
    KInspectorPanel* inspector = new KInspectorPanel(kRightX, y + 212, kRightW, kInspectorH, "Inspector");
    ...
    KAccordion* accordion = new KAccordion(kRightX, y + 212 + kInspectorH + kGap, kRightW, 118, "Accordion");
...
Fl_Group* createInputPage(int x, int y, int w, int h) {
    ...
    warning->setIconColorRole(KIconColorRole::kWarning);
    warning->setIcon("!");
    iconApi->end();
    return finishPage(page);
}
