...
void EtwSessionController::stop() {
    const bool kWasRunning = running_.exchange(false);
    if (!kWasRunning && !workerThread_.joinable()) {
        return;
    }
    stopRequested_.store(true);

    const TRACEHANDLE kOwnedSessionHandle = sessionHandle_.exchange(0);
    if (kOwnedSessionHandle != 0 && !sessionName_.empty()) {
        std::vector<unsigned char> propertiesBuffer = makeTracePropertiesBuffer(sessionName_);
        auto* properties = reinterpret_cast<EVENT_TRACE_PROPERTIES*>(propertiesBuffer.data());
        (void)::ControlTraceW(
            kOwnedSessionHandle,
...
void EtwSessionController::workerLoop() {
    const std::wstring kSessionName = sessionName_;
    const auto kStopOwnedSession = [this, &kSessionName]() {
        const TRACEHANDLE kOwnedSessionHandle = sessionHandle_.exchange(0);
        if (kOwnedSessionHandle == 0 || kSessionName.empty()) {
            return;
        }
        std::vector<unsigned char> propertiesBuffer = makeTracePropertiesBuffer(kSessionName);
        auto* properties = reinterpret_cast<EVENT_TRACE_PROPERTIES*>(propertiesBuffer.data());
        (void)::ControlTraceW(
            kOwnedSessionHandle,
            kSessionName.c_str(),
    ...
    logFile.LoggerName = const_cast<LPWSTR>(kSessionName.c_str());
    ...
    if (kTraceHandle == INVALID_PROCESSTRACE_HANDLE) {
        const ULONG kErrorCode = ::GetLastError();
        kStopOwnedSession();
        const bool kStopped = stopRequested_.load();
        running_.store(false);
        if (!kStopped) {
            publishLastError(win32ErrorText(L"OpenTraceW", kErrorCode));
        }
        return;
    }

    if (stopRequested_.load()) {
        (void)::CloseTrace(kTraceHandle);
        kStopOwnedSession();
        running_.store(false);
        return;
    }

    TRACEHANDLE handles[] = { kTraceHandle };
    const ULONG kStatus = ::ProcessTrace(handles, 1, nullptr, nullptr);
    (void)::CloseTrace(kTraceHandle);
    kStopOwnedSession();

    const bool kStopped = stopRequested_.load();
    running_.store(false);
    if (kStatus != ERROR_SUCCESS && kStatus != ERROR_CANCELLED && !kStopped) {
        publishLastError(win32ErrorText(L"ProcessTrace", kStatus));
    }
    else if (!kStopped) {
        publishStatus(L"ETW session stopped.");
    }
...
bool EtwSessionController::enableProviders() {
    ...
    for (const EtwProviderPreset& provider : filterState_.providers) {
        ...
        const ULONG kStatus = ::EnableTraceEx2(
            sessionHandle_.load(),
        ...
        if (kStatus != ERROR_SUCCESS) {
            publishLastError(win32ErrorText(L"EnableTraceEx2", kStatus));
            continue;
        }
        enabledAny = true;
    }
