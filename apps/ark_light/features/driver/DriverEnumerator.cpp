...
// KrtlProcessModuleInformation mirrors the public structure layout used by
...
struct KrtlProcessModuleInformation {
    PVOID section;
    PVOID mappedBase;
    PVOID imageBase;
    ULONG imageSize;
...
// KrtlProcessModules stores the module count followed by the first entry. The
// buffer returned by NtQuerySystemInformation is contiguous, so the caller can
// walk the array in place.
struct KrtlProcessModules {
    ULONG numberOfModules;
    KrtlProcessModuleInformation modules[1];
};

// KobjectDirectoryInformation mirrors the structure returned by
...
// KobjectBasicInformation captures the counts needed for the read-only
...
std::wstring ansiTextToWide(const char* text, std::size_t length) {
    ...
    const int kNeeded = ::MultiByteToWideChar(CP_ACP, 0, text, static_cast<int>(length), nullptr, 0);
    if (kNeeded <= 0) {
        return {};
    }

    std::wstring wide(static_cast<std::size_t>(kNeeded), L'\0');
    if (::MultiByteToWideChar(CP_ACP, 0, text, static_cast<int>(length), wide.data(), kNeeded) <= 0) {
        return {};
    }
    return wide;
}
...
std::wstring ansiPathFromModule(const KrtlProcessModuleInformation& module) {
    std::size_t length = 0;
...
std::uint64_t parseCompactHex(const std::wstring& text) {
    ...
    wchar_t* end = nullptr;
    const unsigned long long kValue = ::wcstoull(begin, &end, 16);
    return end != begin ? static_cast<std::uint64_t>(kValue) : 0;
}
...
std::wstring resolveKernelImagePathForTrust(const std::wstring& path) {
    ...
    if (_wcsnicmp(path.c_str(), kSystemRootPrefix, _countof(kSystemRootPrefix) - 1) == 0) {
        ...
        const UINT kLength = ::GetWindowsDirectoryW(windowsDirectory, static_cast<UINT>(_countof(windowsDirectory)));
        if (kLength == 0 || kLength >= _countof(windowsDirectory)) {
            return {};
        }
...
std::wstring verifyDriverImageSignature(const std::wstring& displayPath) {
    const std::wstring kLocalPath = resolveKernelImagePathForTrust(displayPath);
    if (kLocalPath.empty()) {
        return L"未解析本地路径";
    }
    if (::GetFileAttributesW(kLocalPath.c_str()) == INVALID_FILE_ATTRIBUTES) {
        return L"文件不可访问";
    }

    WINTRUST_FILE_INFO fileInfo{};
    fileInfo.cbStruct = sizeof(fileInfo);
    fileInfo.pcwszFilePath = kLocalPath.c_str();
    ...
    return L"签名失败 " + ntStatusText(kStatus);
}
...
void applyOverviewAnomalies(
    ...
    for (DriverOverviewRow& row : rows) {
        const std::uint64_t kBase = parseCompactHex(row.baseAddressText);
        const auto kFound = anomalies.find(kBase);
        if (kFound != anomalies.end() && !kFound->second.empty()) {
            row.anomalyText = kFound->second;
        } else {
...
std::wstring leafName(const std::wstring& text) {
    const std::size_t kSlash = text.find_last_of(L"\\/");
    if (kSlash == std::wstring::npos || kSlash + 1 >= text.size()) {
        return text;
    }
    return text.substr(kSlash + 1);
}
...
HANDLE openNtDirectory(const NtLibrary& library, const std::wstring& path) {
    ...
    const LONG kStatus = library.openDirectoryObject(&handle, DIRECTORY_QUERY, &attributes);
    return kStatus >= 0 ? handle : nullptr;
}
...
bool queryBasicCounts(const NtLibrary& library, HANDLE handle, std::wstring& handleCountText, std::wstring& referenceCountText) {
    ...
    const LONG kStatus = library.queryObject(handle, 0, &basic, static_cast<ULONG>(sizeof(basic)), &returnLength);
    if (kStatus < 0) {
        return false;
    }
...
std::wstring querySymbolicLinkTarget(const NtLibrary& library, HANDLE handle) {
    ...
    const LONG kStatus = library.querySymbolicLinkObject(handle, &target, nullptr);
    if (kStatus < 0) {
        return {};
    }
...
DriverObjectRow appendDirectoryRow(const NtLibrary& library, const std::wstring& directoryPath, const std::wstring& name, const std::wstring& typeName) {
    ...
    row.querySucceeded = true;
        ...
        if (row.objectTypeText == L"Directory") {
            ...
            const LONG kStatus = library.openSymbolicLinkObject(&handle, SYMBOLIC_LINK_QUERY, &attributes);
            if (kStatus >= 0 && handle) {
                std::wstring handleCount;
                std::wstring referenceCount;
                if (queryBasicCounts(library, handle, handleCount, referenceCount)) {
...
void appendUniqueDriverQueryName(std::vector<std::wstring>& names, const std::wstring& candidate) {
    ...
    const auto kExists = std::find_if(names.begin(), names.end(), [&](const std::wstring& value) {
...
void appendDriverIntegrityEvidenceRows(
    ...
    for (const ksword::ark::DriverIntegrityEvidenceEntry& entry : query.entries) {
        DriverObjectRow row;
        const std::wstring kClassText = driverIntegrityClassText(entry.evidenceClass);
        const std::wstring kRiskText = driverIntegrityRiskText(entry.riskFlags);
        row.directoryPathText = L"R0 DriverIntegrity";
        row.objectNameText = entry.ownerModule.empty() ? kClassText : entry.ownerModule;
        row.objectTypeText = kClassText;
        ...
        row.statusText = L"risk=" + kRiskText +
        ...
        if (entry.riskFlags != KSWORD_ARK_DRIVER_INTEGRITY_RISK_NONE) {
            appendOverviewAnomaly(anomalies, entry.ownerModuleBase, kClassText + L":" + kRiskText);
        }
    }
}
...
bool enrichDriverIntegrityWithR0(
    ...
    if (!kQuery.io.ok) {
        warnings.push_back(std::wstring(L"R0 DriverIntegrity 查询跳过：") + utf8ToWide(kQuery.io.message));
        return false;
    }

    appendDriverIntegrityEvidenceRows(kQuery, rows, anomalies);
    warnings.push_back(std::wstring(L"R0 DriverIntegrity 只读证据完成：") +
        driverIntegrityStatusText(kQuery.queryStatus) +
        L"，Rows=" + std::to_wstring(kQuery.entries.size()) +
        L"/" + std::to_wstring(kQuery.totalCount) + L"。");
    return kQuery.queryStatus == KSWORD_ARK_DRIVER_INTEGRITY_STATUS_OK ||
        kQuery.queryStatus == KSWORD_ARK_DRIVER_INTEGRITY_STATUS_PARTIAL;
}
...
void enrichDriverObjectsWithR0(std::vector<DriverObjectRow>& rows, std::vector<std::wstring>& warnings) {
    const std::vector<std::wstring> kQueryNames = buildDriverObjectQueryNames(rows);
    if (kQueryNames.empty()) {
    ...
    if (!kCapability.io.ok) {
        warnings.push_back(std::wstring(L"R0 DriverObject 查询跳过：KswordARK 驱动不可用或能力查询失败，Win32=") +
            std::to_wstring(kCapability.io.win32Error) + std::wstring(L"，") + utf8ToWide(kCapability.io.message));
        return;
    }

    const std::size_t kQueryLimit = std::min<std::size_t>(kQueryNames.size(), kMaxR0DriverObjectQueries);
    std::size_t okCount = 0;
    std::size_t partialCount = 0;
    std::size_t failCount = 0;
    for (std::size_t index = 0; index < kQueryLimit; ++index) {
        const std::wstring& driverName = kQueryNames[index];
        ...
        appendDriverObjectSummaryRow(driverName, kQuery, rows);
        if (kQuery.io.ok &&
            ...
            appendDriverObjectMajorRows(driverName, kQuery, rows);
            appendDriverObjectDeviceRows(driverName, kQuery, rows);
    ...
    std::wstring summary = std::wstring(L"R0 DriverObject 查询完成：OK=") + std::to_wstring(okCount) +
        ...
        std::wstring(L"，Queued=") + std::to_wstring(kQueryNames.size()) + std::wstring(L"。");
    if (kQueryNames.size() > kQueryLimit) {
        summary += std::wstring(L"为避免刷新阻塞，本次只查询前 ") + std::to_wstring(kQueryLimit) + std::wstring(L" 项。");
    }
...
bool queryModuleInformation(std::vector<DriverOverviewRow>& rows, std::wstring& diagnosticText) {
    ...
    const auto* modules = reinterpret_cast<const KrtlProcessModules*>(buffer.data());
    rows.reserve(modules->numberOfModules);
    for (ULONG index = 0; index < modules->numberOfModules; ++index) {
        const KrtlProcessModuleInformation& module = modules->modules[index];
        DriverOverviewRow row;
        row.driverName = ansiPathFromModule(module);
        row.driverName = leafName(row.driverName);
        const std::uint64_t kBaseAddress = reinterpret_cast<std::uint64_t>(module.imageBase);
        const std::uint64_t kEndAddress = kBaseAddress + static_cast<std::uint64_t>(module.imageSize);
        row.baseAddressText = formatHexAddress(kBaseAddress);
        row.memoryRangeText = formatHexAddress(kBaseAddress) + L"-" + formatHexAddress(kEndAddress);
...
bool queryPsapiModules(std::vector<DriverOverviewRow>& rows, std::wstring& diagnosticText) {
    ...
    const std::size_t kCount = needed / sizeof(LPVOID);
    rows.reserve(kCount);
    for (std::size_t index = 0; index < kCount; ++index) {
        ...
        const std::uint64_t kBaseAddress = reinterpret_cast<std::uint64_t>(base);
        row.baseAddressText = formatHexAddress(kBaseAddress);
        row.memoryRangeText = formatHexAddress(kBaseAddress) + L"-未知";
        row.sizeText = L"未知";
...
void queryObjectDirectory(
    ...
    for (;;) {
        ULONG returnLength = 0;
        const LONG kStatus = library.queryDirectoryObject(
            directory,
            buffer.data(),
        ...
        const auto* entry = reinterpret_cast<const KobjectDirectoryInformation*>(buffer.data());
        if (!entry->name.Buffer || entry->name.Length == 0) {
            continue;
        }

        const std::wstring kName = wideText(entry->name);
        const std::wstring kTypeName = wideText(entry->typeName);
        rows.push_back(appendDirectoryRow(library, directoryPath, kName, kTypeName));
    }
...
DriverEnumerationResult enumerateDriverSnapshot() {
    ...
    const NtLibrary kLibrary = ntLibraryHandle();
    if (!kLibrary.openDirectoryObject || !kLibrary.queryDirectoryObject) {
    ...
    for (const std::wstring& root : kRoots) {
        queryObjectDirectory(kLibrary, root, result.objectRows, warnings);
    }
