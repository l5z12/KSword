...
RECT splitPreview(RECT rc, DockManager::DropKind kind) {
    switch (kind) {
    case DockManager::DropKind::kPaneLeft:
    case DockManager::DropKind::kRootLeft:
        rc.right = rc.left + maxInt(1, width(rc) / 3);
        break;
    case DockManager::DropKind::kPaneRight:
    case DockManager::DropKind::kRootRight:
        rc.left = rc.right - maxInt(1, width(rc) / 3);
        break;
    case DockManager::DropKind::kPaneTop:
    case DockManager::DropKind::kRootTop:
        rc.bottom = rc.top + maxInt(1, height(rc) / 3);
        break;
    case DockManager::DropKind::kPaneBottom:
    case DockManager::DropKind::kRootBottom:
        rc.top = rc.bottom - maxInt(1, height(rc) / 3);
        break;
    case DockManager::DropKind::kTab:
...
bool horizontalSplit(DockManager::DropKind kind) {
    return kind == DockManager::DropKind::kPaneLeft || kind == DockManager::DropKind::kPaneRight ||
        kind == DockManager::DropKind::kRootLeft || kind == DockManager::DropKind::kRootRight;
}
...
bool incomingFirst(DockManager::DropKind kind) {
    return kind == DockManager::DropKind::kPaneLeft || kind == DockManager::DropKind::kPaneTop ||
        kind == DockManager::DropKind::kRootLeft || kind == DockManager::DropKind::kRootTop;
}
...
int DockManager::addDock(DockPosition position, const std::wstring& title, HWND content) {
    ...
    const int kIndex = static_cast<int>(docks_.size()) - 1;
    setDockParentToHost(kIndex);
    if (!root_ || nodeEmpty(root_.get())) {
        root_ = makeLeaf(kIndex);
    } else if (position == DockPosition::kLeft) {
        splitRootWithDock(kIndex, DropKind::kRootLeft);
    } else if (position == DockPosition::kRight) {
        splitRootWithDock(kIndex, DropKind::kRootRight);
    } else if (position == DockPosition::kTop) {
        splitRootWithDock(kIndex, DropKind::kRootTop);
    } else if (position == DockPosition::kBottom) {
        splitRootWithDock(kIndex, DropKind::kRootBottom);
    } else {
        appendDockToLeaf(firstLeaf(), kIndex);
    }
    activeIndex_ = kIndex;
    layoutChildren();
...
void DockManager::closeDock(int index) {
    ...
    const bool kClosedWasActive = activeIndex_ == index;
    if (docks_[index]->floating()) {
    ...
    const bool kActiveStillVisible = activeIndex_ >= 0 && activeIndex_ < static_cast<int>(docks_.size()) &&
        docks_[activeIndex_] && docks_[activeIndex_]->visible();
    if (kClosedWasActive || !kActiveStillVisible) {
        activeIndex_ = -1;
...
LRESULT DockManager::handleMessage(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_MOUSEMOVE: {
        ...
        TabHit hit = hitTestTab(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
        const int kNextHot = hit.valid ? hit.dockIndex : -1;
        if (kNextHot != hotTabDock_) {
            hotTabDock_ = kNextHot;
    ...
    case WM_RBUTTONUP: {
        ...
        if (hit.valid) {
            ...
            POINT pt{ GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam) };
            ::ClientToScreen(hwnd, &pt);
            const int kCmd = ::TrackPopupMenu(menu, TPM_RETURNCMD | TPM_RIGHTBUTTON, pt.x, pt.y, 0, hwnd, nullptr);
            ::DestroyMenu(menu);
            if (kCmd == 1) {
...
void DockManager::paintLeaf(HDC dc, const DockNode* leaf) const {
    ...
    for (int ordinal = 0; ordinal < static_cast<int>(leaf->docks.size()); ++ordinal) {
        const int kDockIndex = leaf->docks[ordinal];
        if (kDockIndex < 0 || kDockIndex >= static_cast<int>(docks_.size()) || !docks_[kDockIndex]->visible()) {
            continue;
        }
        int tabWidth = tabWidthForTitle(dc, docks_[kDockIndex]->title());
        tabWidth = minInt(tabWidth, maxInt(kTabMinWidth, tabs.right - x));
        RECT tab{ x, tabs.top, x + tabWidth, tabs.bottom };
        const bool kActive = kDockIndex == leaf->activeDock;
        const bool kHot = kDockIndex == hotTabDock_;
        HBRUSH brush = kActive ? ksword::ui::appTheme().accentBrush() : ksword::ui::appTheme().panelBrush();
        ...
        ksword::ui::drawTextLine(dc, docks_[kDockIndex]->title(), tab,
            kActive ? RGB(255, 255, 255) : ksword::ui::appTheme().textColor,
...
void DockManager::paintDropGuides(HDC dc) const {
    ...
    RECT rc{};
    ::GetClientRect(hwnd_, &rc);
    const bool kHasHover = hoverTarget_ && hoverTarget_->valid;
    if (kHasHover) {
    ...
    const RECT kGuidePane = guideLeaf ? guideLeaf->rect : rc;
    RECT tab = centerTabGuide(kGuidePane);
    RECT left = centerLeftGuide(kGuidePane);
    RECT right = centerRightGuide(kGuidePane);
    RECT top = centerTopGuide(kGuidePane);
    RECT bottom = centerBottomGuide(kGuidePane);
    drawDropBox(dc, tab, L"TAB", kHasHover && hoverTarget_->kind == DropKind::kTab);
    drawDropBox(dc, left, L"L", kHasHover && hoverTarget_->kind == DropKind::kPaneLeft);
    drawDropBox(dc, right, L"R", kHasHover && hoverTarget_->kind == DropKind::kPaneRight);
    drawDropBox(dc, top, L"T", kHasHover && hoverTarget_->kind == DropKind::kPaneTop);
    drawDropBox(dc, bottom, L"B", kHasHover && hoverTarget_->kind == DropKind::kPaneBottom);
    ...
    drawDropBox(dc, rootLeft, L"ROOT L", kHasHover && hoverTarget_->kind == DropKind::kRootLeft);
    drawDropBox(dc, rootRight, L"ROOT R", kHasHover && hoverTarget_->kind == DropKind::kRootRight);
    drawDropBox(dc, rootTop, L"ROOT T", kHasHover && hoverTarget_->kind == DropKind::kRootTop);
    drawDropBox(dc, rootBottom, L"ROOT B", kHasHover && hoverTarget_->kind == DropKind::kRootBottom);
}
...
void DockManager::layoutNode(DockNode* node, const RECT& rect) {
    ...
    if (node->horizontal) {
        const int kSplitX = rect.left + static_cast<int>(width(rect) * node->ratio);
        RECT first{ rect.left, rect.top, kSplitX - kSplitterSize / 2, rect.bottom };
        RECT second{ kSplitX + kSplitterSize / 2, rect.top, rect.right, rect.bottom };
        layoutNode(node->first.get(), first);
        layoutNode(node->second.get(), second);
    } else {
        const int kSplitY = rect.top + static_cast<int>(height(rect) * node->ratio);
        RECT first{ rect.left, rect.top, rect.right, kSplitY - kSplitterSize / 2 };
        RECT second{ rect.left, kSplitY + kSplitterSize / 2, rect.right, rect.bottom };
        layoutNode(node->first.get(), first);
        layoutNode(node->second.get(), second);
    }
}
...
void DockManager::collapseEmptySplits(std::unique_ptr<DockNode>& node) {
    ...
    const bool kFirstEmpty = nodeEmpty(node->first.get());
    const bool kSecondEmpty = nodeEmpty(node->second.get());
    if (kFirstEmpty && !kSecondEmpty) {
        node = std::move(node->second);
    } else if (!kFirstEmpty && kSecondEmpty) {
...
DockManager::TabHit DockManager::hitTestTab(int x, int y) const {
    ...
    for (int ordinal = 0; ordinal < static_cast<int>(leaf->docks.size()); ++ordinal) {
        const int kDockIndex = leaf->docks[ordinal];
        if (kDockIndex < 0 || kDockIndex >= static_cast<int>(docks_.size())) {
            continue;
        }
        int tabWidth = tabWidthForTitle(dc, docks_[kDockIndex]->title());
...
void DockManager::updateTabDrag(POINT screenPoint) {
    ...
    const int kDx = screenPoint.x - dragStartScreen_.x;
    const int kDy = screenPoint.y - dragStartScreen_.y;
    if (!draggingTab_ && (kDx * kDx + kDy * kDy) >= kDragThreshold * kDragThreshold) {
        draggingTab_ = true;
...
void DockManager::finishTabDrag(POINT screenPoint) {
    ...
    const bool kWasDragging = draggingTab_;
    const int kDockIndex = dragDockIndex_;
    if (::GetCapture() == hwnd_) {
        capturedTab_ = false;
        ::ReleaseCapture();
    } else {
    ...
    const DropTarget kTarget = dropTargetFromScreen(screenPoint);
    const bool kDocked = kWasDragging && moveDockToTarget(kDockIndex, kTarget);
    if (!kWasDragging) {
        activateDock(kDockIndex);
    }
    if (kWasDragging && !kDocked && kDockIndex >= 0 && kDockIndex < static_cast<int>(docks_.size()) &&
        docks_[kDockIndex]->floating()) {
        ::SetWindowPos(docks_[kDockIndex]->floatingHost(), HWND_NOTOPMOST, 0, 0, 0, 0,
...
void DockManager::floatDock(int dockIndex, POINT screenPoint, bool noActivate) {
    ...
    const POINT kOffset = noActivate ? dragWindowOffset_ : POINT{ 96, 18 };
    ...
    HWND host = ::CreateWindowExW(WS_EX_TOOLWINDOW, kFloatingClass, docks_[dockIndex]->title().c_str(),
        ...
        screenPoint.x - kOffset.x, screenPoint.y - kOffset.y, kFloatingWidth, kFloatingHeight,
    ...
    ::SetWindowPos(host, noActivate ? HWND_TOPMOST : HWND_TOP, screenPoint.x - kOffset.x,
        screenPoint.y - kOffset.y, kFloatingWidth, kFloatingHeight,
...
void DockManager::finishFloatingMove(FloatingDock* floating, POINT screenPoint) {
    ...
    const DropTarget kTarget = dropTargetFromScreen(screenPoint);
    if (kTarget.valid) {
        moveDockToTarget(floating->dockIndex, kTarget);
    }
    hoverTarget_.reset();
    hideDropOverlay();
    layoutChildren();
...
bool DockManager::moveDockToTarget(int dockIndex, const DropTarget& target) {
    ...
    bool ok = false;
    if (target.kind == DropKind::kTab) {
        ok = insertDockAsTab(dockIndex, target.leafId);
    } else if (target.kind == DropKind::kPaneLeft || target.kind == DropKind::kPaneRight ||
        target.kind == DropKind::kPaneTop || target.kind == DropKind::kPaneBottom) {
