    ...
    const KSWORD_ARK_QUERY_HVM_RESPONSE& response) const
{
    ...
    detail += kernelText(
        ...
        .arg(QString::number(
            ...
            16).toUpper())
        // Loopback and cumulative counts appear last: QString::arg consumes placeholders by index, independent
        // of text position; inserting in the middle forces renumbering of all subsequent placeholders.
...
QString KernelHvmTab::featureText(const std::uint64_t flags)
{
    ...
    static constexpr std::array<FeatureName, 43> kNames{{
...
/*
 * Convert the current nested ladder level into human-readable text.
 *
 * This field is always present in query responses, yet the UI displays nothing—only the CLI shows how far
 * nesting has progressed. Consequently, the question "Did enabling the resident mode actually turn on nesting?"
 * **cannot be answered** in the GUI, despite being the sole metric users need to check daily for this feature.
 *
 * The ladder is monotonic but **not persistent**: L2_ACTIVE holds only for the instant L2 is actually running;
 * the next VMXOFF reverts it to DISPATCH_READY. Thus, reading DISPATCH_READY in a single poll indicates 'L2 is
 * not running at this moment', not 'L2 has never run'—the latter requires checking that monotonic counter.
 */
...
QString KernelHvmTab::executionStageText(const std::uint32_t stage)
{
    ...
    // Do not collapse unknown indices to 'Unknown': When a new driver adds stages but the UI hasn't caught up, the numeric index is
    // the only clue to correlate the symptom with the driver change; removing it deletes the starting point for troubleshooting.
