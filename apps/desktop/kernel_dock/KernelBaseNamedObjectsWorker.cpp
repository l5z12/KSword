...
// Purpose:
// 1) R3 performs read-only enumeration of BaseNamedObjects-related object directories;
// 2) Aggregate one layer of named objects from Global, current Session, and discoverable Sessions;
// 3) For SymbolicLink, additionally resolve the target; for Directory, only mark as enumerable.
// ============================================================
...
namespace
{
    ...
    // - Input: None; dynamically populated by loadNtApi;
    // - Processing: Save the ntdll entry points required by this worker.
    ...
    // ScopedNtHandle：
    // - Input: Nt* opened HANDLE.
    // - Processing: CloseHandle on destruction;
    // - Return behavior: returns no value; solely responsible for lifecycle management.
    ...
    {
        const QString kDirectoryText = normalizeObjectPath(directoryPathText);
        if (kDirectoryText == QStringLiteral("\\"))
        {
            return QStringLiteral("\\%1").arg(objectNameText);
        }
        return kDirectoryText.endsWith('\\')
            ? kDirectoryText + objectNameText
            : kDirectoryText + QStringLiteral("\\") + objectNameText;
    }
    ...
    {
        const std::wstring kPathWideText = normalizeObjectPath(directoryPathText).toStdWString();
        UNICODE_STRING objectName{};
        initializeUnicodeString(objectName, kPathWideText);
    ...
    NTSTATUS openSymbolicLink(
        ...
        ScopedNtHandle& handleOut)
    {
        const std::wstring kPathWideText = normalizeObjectPath(symbolicPathText).toStdWString();
        UNICODE_STRING objectName{};
        initializeUnicodeString(objectName, kPathWideText);
    ...
    {
        const QString kTrimmedText = textValue.trimmed();
        if (kTrimmedText.isEmpty())
        {
            return false;
        }
        for (const QChar kSingleChar : kTrimmedText)
        {
    ...
    {
        const QString kTypeText = objectTypeText.trimmed();
        if (kTypeText.compare(QStringLiteral("Event"), Qt::CaseInsensitive) == 0) return QStringLiteral("Event");
        if (kTypeText.compare(QStringLiteral("Mutant"), Qt::CaseInsensitive) == 0) return QStringLiteral("Mutant");
        if (kTypeText.compare(QStringLiteral("Semaphore"), Qt::CaseInsensitive) == 0) return QStringLiteral("Semaphore");
        if (kTypeText.compare(QStringLiteral("Section"), Qt::CaseInsensitive) == 0) return QStringLiteral("Section");
        if (kTypeText.compare(QStringLiteral("Timer"), Qt::CaseInsensitive) == 0) return QStringLiteral("Timer");
        if (kTypeText.compare(QStringLiteral("Job"), Qt::CaseInsensitive) == 0) return QStringLiteral("Job");
        if (kTypeText.compare(QStringLiteral("Directory"), Qt::CaseInsensitive) == 0) return QStringLiteral("Directory");
        if (kTypeText.compare(QStringLiteral("SymbolicLink"), Qt::CaseInsensitive) == 0) return QStringLiteral("SymbolicLink");
    ...
    std::vector<unsigned long> discoverSessionIds(const NtBaseNamedObjectsApi& api)
    {
        ...
        if (enumerateDirectory(api, QStringLiteral("\\Sessions"), sessionRecords, statusText))
        {
            for (const DirectoryRecord& record : sessionRecords)
            {
                ...
                bool parseOk = false;
                const unsigned long kSessionId = record.nameText.toULong(&parseOk);
                if (parseOk)
                {
                    sessionIds.insert(kSessionId);
                }
            }
        }
    ...
    std::vector<DirectorySpec> buildDirectorySpecs(const NtBaseNamedObjectsApi& api)
    {
        ...
        const bool kHasCurrentSession =
            ::ProcessIdToSessionId(::GetCurrentProcessId(), &currentSessionId) != FALSE;

        const std::vector<unsigned long> kSessionIds = discoverSessionIds(api);
        for (const unsigned long kSessionId : kSessionIds)
        {
            const bool kIsCurrentSession = kHasCurrentSession && kSessionId == currentSessionId;
            specs.push_back(DirectorySpec{
                kIsCurrentSession
                    ? QStringLiteral("Current Session %1").arg(kSessionId)
                    : QStringLiteral("Session %1").arg(kSessionId),
                QStringLiteral("\\Sessions\\%1\\BaseNamedObjects").arg(kSessionId),
                kSessionId,
                true
                });
        }
    ...
    QString& errorTextOut)
{
    ...
    for (const DirectorySpec& spec : buildDirectorySpecs(api))
    {
        ...
        const bool kEnumOk = enumerateDirectory(api, spec.pathText, records, directoryStatusText);
        if (!kEnumOk)
        {
