...
public:
    ...
    KButton(int x, int y, int w, int h, const char* label = nullptr, KButtonType type = kKbuttonHeavy);
...
public:
    ...
    KSeparator(int x, int y, int w, int h, KSeparatorOrientation orientation = KSeparatorOrientation::kHorizontal);
...
private:
    ...
    void drawImageInContent(int contentX, int contentY, int contentW, int contentH);

private:
    Fl_Image* image_;
    Fl_Shared_Image* sharedImage_;
    KImageViewMode displayMode_;
    Fl_Color backgroundColor_;
...
public:
    typedef void (*ContextMenuCallback)(KTable* table, int row, int col, void* userData);
...
public:
    ...
    void setColHeaderLabel(int col, const char* label);

    // Returns a stable header string or an empty string.
    const char* colHeaderLabel(int col) const;
    ...
    void setRowContextMenuCallback(int row, ContextMenuCallback callback, void* userData = nullptr);

    // Registers a cell-level context callback; returns no value.
    void setCellContextMenuCallback(int row, int col, ContextMenuCallback callback, void* userData = nullptr);

    // Writes the last context-menu root-screen position into x/y; returns no value.
    void contextMenuPosition(int& x, int& y) const;
...
private:
    ...
    bool triggerContextMenuCallback(int row, int col);

private:
    std::vector<std::string> colLabels_;
    std::vector<std::vector<std::string>> cells_;
    std::vector<ContextMenuBinding> rowContextMenuCallbacks_;
    std::vector<std::vector<ContextMenuBinding>> cellContextMenuCallbacks_;
    int rowHeight_;
    int colWidth_;
    int contextMenuX_;
    int contextMenuY_;
...
KButton* kCreateButton(int x, int y, int w, int h, const char* label = nullptr, KButtonType type = kKbuttonHeavy);
...
KSeparator* kCreateSeparator(int x, int y, int w, int h, KSeparatorOrientation orientation = KSeparatorOrientation::kHorizontal);
...
KImageView* kCreateImageViewFromFile(int x, int y, int w, int h, const char* path, const char* label = nullptr, KImageFitMode mode = KImageFitMode::kContain);
