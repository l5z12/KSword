...
namespace ksword::ark
{
        ...
        const unsigned long maxEntries) const
    {
        ...
        for (std::size_t index = 0U; index < kParsedCount; ++index)
        {
            ...
            if (kEntryOffset + sizeof(KSWORD_ARK_KEYBOARD_HOTKEY_ENTRY) > responseBuffer.size())
            {
                break;
            }

            const auto* sourceEntry =
                reinterpret_cast<const KSWORD_ARK_KEYBOARD_HOTKEY_ENTRY*>(responseBuffer.data() + kEntryOffset);
            ...
            row.detail = detail::readFixedString(sourceEntry->detail, KSWORD_ARK_KEYBOARD_DETAIL_CHARS);
        ...
        const unsigned long newVirtualKey) const
    {
        ...
        // Force the establishment of a Win32 USER context on the current GUI calling thread; the driver will reject worker-only calls.
        ...
        const unsigned long maxEntries) const
    {
        ...
        for (std::size_t index = 0U; index < kParsedCount; ++index)
        {
            const std::size_t kEntryOffset =
                ...
                (index * static_cast<std::size_t>(responseHeader->entrySize));
            if (kEntryOffset + sizeof(KSWORD_ARK_KEYBOARD_HOOK_ENTRY) > responseBuffer.size())
            {
                break;
            }

            const auto* sourceEntry =
                reinterpret_cast<const KSWORD_ARK_KEYBOARD_HOOK_ENTRY*>(responseBuffer.data() + kEntryOffset);
