...
// a numeric error code; output is either "OK" or "Error <code>".
...
DirectoryEnumerationResult FileSystemEnumerator::enumerateDrives() const {
    ...
    const DWORD kMask = ::GetLogicalDrives();
    const DWORD kBufferChars = ::GetLogicalDriveStringsW(0, nullptr);
    if (kMask == 0 && kBufferChars == 0) {
        result.errorCode = ::GetLastError();
        result.statusText = makeStatusText(result.errorCode);
        return result;
    }

    std::vector<wchar_t> buffer(kBufferChars + 2, L'\0');
    const DWORD kWritten = ::GetLogicalDriveStringsW(static_cast<DWORD>(buffer.size()), buffer.data());
    if (kWritten == 0 || kWritten >= buffer.size()) {
        result.errorCode = ::GetLastError();
        result.statusText = makeStatusText(result.errorCode);
        return result;
    }
    ...
    for (const wchar_t* drive = buffer.data(); drive && *drive; drive += std::wcslen(drive) + 1) {
        FileEntry entry;
        entry.kind = FileEntryKind::kDrive;
        entry.name = drive;
...
DirectoryEnumerationResult FileSystemEnumerator::enumerateDirectory(const std::wstring& directory) const {
    ...
    const std::wstring kPattern = PathNavigator::makeSearchPattern(directory);
    WIN32_FIND_DATAW data{};
    HANDLE find = ::FindFirstFileW(kPattern.c_str(), &data);
    ...
    do {
        ...
        FileEntry entry;
        entry.name = kName;
        entry.fullPath = PathNavigator::joinChildPath(directory, kName);
        entry.attributes = data.dwFileAttributes;
        entry.kind = (data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ? FileEntryKind::kDirectory : FileEntryKind::kFile;
    ...
    ::FindClose(find);
    if (kLastError != ERROR_NO_MORE_FILES) {
        result.errorCode = kLastError;
        result.statusText = makeStatusText(kLastError);
        return result;
    }
