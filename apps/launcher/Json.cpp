...
JsonValue::JsonValue(std::string value) : type_(Type::kString), string_(std::move(value)) {}
...
const JsonValue* JsonValue::get(const char* name) const {
    if (!name || type_ != Type::kObject) return nullptr;
...
private:
    ...
    bool parseValue(JsonValue* output) {
        ...
        const char kCh = text_[position_];
        if (kCh == '{') return parseObject(output);
        if (kCh == '[') return parseArray(output);
        if (kCh == '"') {
            std::string value;
        ...
        if (kCh == 't' && consumeLiteral("true")) { *output = JsonValue(true); return true; }
        if (kCh == 'f' && consumeLiteral("false")) { *output = JsonValue(false); return true; }
        if (kCh == 'n' && consumeLiteral("null")) { *output = JsonValue(); return true; }
        return parseNumber(output);
    }

    bool consumeLiteral(const char* literal) {
        const size_t kLength = std::strlen(literal);
        if (text_.compare(position_, kLength, literal) != 0) return false;
        position_ += kLength;
        return true;
    }
    ...
    bool parseString(std::string* output) {
        ...
        while (position_ < text_.size()) {
            const unsigned char kCh = static_cast<unsigned char>(text_[position_++]);
            if (kCh == '"') return true;
            if (kCh < 0x20) return fail("control character in string");
            if (kCh != '\\') { output->push_back(static_cast<char>(kCh)); continue; }
            if (position_ >= text_.size()) return fail("unfinished escape");
            const char kEscaped = text_[position_++];
            switch (kEscaped) {
            ...
            case 'u': {
                // The manifest uses only ASCII fields; BMP escapes are still accepted to facilitate report parsing.
                if (position_ + 4 > text_.size()) return fail("short unicode escape");
                unsigned value = 0;
                for (int index = 0; index < 4; ++index) {
                    const char kDigit = text_[position_++];
                    const int kNibble = std::isdigit(static_cast<unsigned char>(kDigit)) ? kDigit - '0' :
                        (kDigit >= 'a' && kDigit <= 'f') ? kDigit - 'a' + 10 :
                        (kDigit >= 'A' && kDigit <= 'F') ? kDigit - 'A' + 10 : -1;
                    if (kNibble < 0) return fail("invalid unicode escape");
                    value = (value << 4) | static_cast<unsigned>(kNibble);
                }
    ...
    bool parseNumber(JsonValue* output) {
        ...
        if (kStart == position_) return fail("expected value");
        char* end = nullptr;
        const double kValue = std::strtod(text_.c_str() + kStart, &end);
        if (end != text_.c_str() + position_) return fail("invalid number");
        *output = JsonValue(kValue);
        return true;
    }
