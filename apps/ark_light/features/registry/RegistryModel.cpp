...
bool startsWithNoCase(const std::wstring& text, const wchar_t* prefix) {
    ...
    return text.size() >= kLength && _wcsnicmp(text.c_str(), prefix, kLength) == 0;
}
...
bool parseUnsigned(const std::wstring& text, unsigned long long& value) {
    ...
    wchar_t* end = nullptr;
    const int kBase = startsWithNoCase(kTrimmed, L"0x") ? 16 : 10;
    value = std::wcstoull(kTrimmed.c_str(), &end, kBase);
    return end != kTrimmed.c_str() && *end == L'\0';
}
...
RegistryPathInfo parseRegistryPath(const std::wstring& text) {
    ...
    if (startsWithNoCase(path, L"\\REGISTRY\\MACHINE")) {
        ...
        for (const RootAlias& alias : kAliases) {
            if (!startsWithNoCase(path, alias.alias)) {
                continue;
            }
            const std::size_t kAliasLength = std::wcslen(alias.alias);
            if (path.size() > kAliasLength && path[kAliasLength] != L'\\') {
                continue;
            }
            info.root = alias.root;
            info.rootText = alias.rootText;
            info.subKey = path.size() > kAliasLength ? path.substr(kAliasLength) : std::wstring();
...
std::wstring rootRegistryPath(const std::wstring& text) {
    ...
    const RegistryPathInfo kParsed = parseRegistryPath(text);
    if (!kParsed.valid) {
        return {};
    }
    return kParsed.rootText;
}
...
std::wstring parentRegistryPath(const std::wstring& text) {
    ...
    const RegistryPathInfo kParsed = parseRegistryPath(text);
    if (!kParsed.valid || kParsed.subKey.empty()) {
        return kParsed.valid ? kParsed.displayPath : std::wstring();
    }
    const std::size_t kSlash = kParsed.subKey.find_last_of(L'\\');
    if (kSlash == std::wstring::npos) {
        return kParsed.rootText;
    }
    return kParsed.rootText + L"\\" + kParsed.subKey.substr(0, kSlash);
}
...
std::wstring formatRegistryData(const std::uint32_t type, const std::vector<std::uint8_t>& data) {
    ...
    if (type == REG_SZ || type == REG_EXPAND_SZ) {
        ...
        const std::size_t kChars = data.size() / sizeof(wchar_t);
        std::wstring value(text, text + kChars);
    ...
    if (type == REG_DWORD && data.size() >= sizeof(std::uint32_t)) {
        const std::uint32_t kValue = static_cast<std::uint32_t>(data[0]) |
            (static_cast<std::uint32_t>(data[1]) << 8) |
            (static_cast<std::uint32_t>(data[2]) << 16) |
            (static_cast<std::uint32_t>(data[3]) << 24);
        std::wostringstream stream;
        stream << kValue << L" (0x" << std::uppercase << std::hex << kValue << L")";
        return stream.str();
    }
...
bool parseRegistryDataText(const std::uint32_t type, const std::wstring& text, std::vector<std::uint8_t>& bytes, std::wstring& errorText) {
    ...
    while (stream >> token) {
        ...
        wchar_t* end = nullptr;
        const unsigned long kValue = std::wcstoul(token.c_str(), &end, 16);
        if (end == token.c_str() || *end != L'\0' || kValue > 0xFFUL) {
            errorText = L"Binary data contains an invalid hex byte.";
            return false;
        }
        bytes.push_back(static_cast<std::uint8_t>(kValue));
    }
    return true;
}
