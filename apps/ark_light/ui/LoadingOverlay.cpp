...
LRESULT CALLBACK loadingOverlayProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT: {
        ...
        const int kCenterX = (rect.left + rect.right) / 2;
        const int kCenterY = std::max(rect.top + 22, (rect.top + rect.bottom) / 2 - 14);
        for (unsigned int index = 0; index < 12U; ++index) {
            const unsigned int kAge = (index + 12U - (state ? state->phase : 0U)) % 12U;
            const int kIntensity = 70 + static_cast<int>((11U - kAge) * 14U);
            HPEN dot = ::CreatePen(PS_SOLID, 3, RGB(kIntensity / 2, kIntensity, std::min(255, kIntensity + 35)));
            const double kAngle = static_cast<double>(index) * 3.14159265358979323846 / 6.0;
            const int kX = kCenterX + static_cast<int>(std::cos(kAngle) * 12.0);
            const int kY = kCenterY + static_cast<int>(std::sin(kAngle) * 12.0);
            HGDIOBJ old = ::SelectObject(dc, dot);
            ::MoveToEx(dc, kX, kY, nullptr);
            ::LineTo(dc, kX + 1, kY + 1);
            ::SelectObject(dc, old);
            ::DeleteObject(dot);
        }

        const std::wstring kText = state ? state->message : L"正在加载…";
        const int kTextBottom = std::min(static_cast<int>(rect.bottom - 8), kCenterY + 52);
        RECT textRect{ rect.left + 12, kCenterY + 24, rect.right - 12, kTextBottom };
        drawTextLine(dc, kText, textRect, appTheme().mutedTextColor, systemUiFont(), DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
        ::EndPaint(hwnd, &paint);
        return 0;
    }
    case WM_NCDESTROY:
