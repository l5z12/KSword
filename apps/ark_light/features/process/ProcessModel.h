...
struct ProcessDisplayRow {
    ...
    ProcessFriendlyGroup group = ProcessFriendlyGroup::kBackground;
    int depth = 0;
...
public:
    ...
    // textForColumn: Returns text based on stable logical column IDs for ListView rendering after column selection.
    // Input: displayRow and column. An empty output indicates no valid process row; missing collected fields are marked as unavailable by the column formatter.
