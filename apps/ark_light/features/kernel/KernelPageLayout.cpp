...
const std::vector<TopLevelTabSpec> kOriginalTopLevelTabs = {
    { L"动态偏移", KernelFeatureId::kPdbProfileStatus },
    { L"对象命名空间", KernelFeatureId::kObjectNamespaceOverview },
    { L"IPC", KernelFeatureId::kIpcSummary },
    { L"原子表遍历", KernelFeatureId::kAtomTable },
    { L"历史NtQuery", KernelFeatureId::kNtQueryLegacy },
    { L"SSDT遍历", KernelFeatureId::kSsdt },
    { L"SSSDT解析", KernelFeatureId::kShadowSsdt },
    { L"Inline Hook 审计", KernelFeatureId::kInlineHook },
    { L"IAT/EAT 钩子检测", KernelFeatureId::kIatEatHook },
    { L"Hook审计", KernelFeatureId::kHookAuditSummary },
    { L"驱动回调", KernelFeatureId::kCallbackIntercept },
    { L"回调遍历", KernelFeatureId::kCallbackEnumeration },
    { L"内核可执行内存", KernelFeatureId::kKernelExecutableMemory },
    { L"CrossView", KernelFeatureId::kProcessCrossView },
    { L"定时器/DPC", KernelFeatureId::kKernelTimerDpc },
    { L"IOCTL 派遣表", KernelFeatureId::kIoctlRegistry },
    { L"内核修改审计", KernelFeatureId::kMutationAudit },
    { L"键盘", KernelFeatureId::kKeyboardHotkeys },
};

const std::vector<ObjectNamespaceTabSpec> kObjectNamespaceTabs = {
    { L"总览", KernelFeatureId::kObjectNamespaceOverview },
    { L"目录递归", KernelFeatureId::kObjectDirectoryRecursive },
    { L"命名管道", KernelFeatureId::kNamedPipe },
    { L"BaseNamedObjects", KernelFeatureId::kBaseNamedObjects },
    { L"符号链接", KernelFeatureId::kSymbolicLink },
    { L"设备与驱动", KernelFeatureId::kDeviceDriverObjects },
    { L"对象类型", KernelFeatureId::kObjectTypeMatrix },
    { L"通信端点", KernelFeatureId::kCommunicationEndpoint },
};

const std::vector<ObjectNamespaceTabSpec> kDynDataTabs = {
    { L"PDB/Profile", KernelFeatureId::kPdbProfileStatus },
    { L"DynData字段", KernelFeatureId::kDynData },
    { L"Capability", KernelFeatureId::kDynDataCapabilities },
    { L"驱动状态", KernelFeatureId::kDriverStatus },
};

const std::vector<ObjectNamespaceTabSpec> kCrossViewTabs = {
    { L"进程矩阵", KernelFeatureId::kProcessCrossView },
    { L"线程矩阵", KernelFeatureId::kThreadCrossView },
    { L"CID表", KernelFeatureId::kCidTableSummary },
};

const std::vector<ObjectNamespaceTabSpec> kIpcTabs = {
    { L"IPC汇总", KernelFeatureId::kIpcSummary },
    { L"命名管道", KernelFeatureId::kNamedPipe },
    { L"通信端点", KernelFeatureId::kCommunicationEndpoint },
};

const std::vector<ObjectNamespaceTabSpec> kHookAuditTabs = {
    { L"审计摘要", KernelFeatureId::kHookAuditSummary },
    { L"SSDT", KernelFeatureId::kSsdt },
    { L"ShadowSSDT", KernelFeatureId::kShadowSsdt },
    { L"Inline", KernelFeatureId::kInlineHook },
    { L"IAT/EAT", KernelFeatureId::kIatEatHook },
    { L"回调遍历", KernelFeatureId::kCallbackEnumeration },
};

const std::vector<ObjectNamespaceTabSpec> kKeyboardTabs = {
    { L"热键", KernelFeatureId::kKeyboardHotkeys },
    { L"钩子", KernelFeatureId::kKeyboardHooks },
};
...
KernelPageLayoutKind layoutKindForFeature(const KernelFeatureId featureId) {
    switch (featureId) {
    case KernelFeatureId::kObjectNamespaceOverview:
        return KernelPageLayoutKind::kTreeWithPropertyTable;
    case KernelFeatureId::kObjectDirectoryRecursive:
        return KernelPageLayoutKind::kTree;
    case KernelFeatureId::kNamedPipe:
        return KernelPageLayoutKind::kTableWithDetail;
    ...
    case KernelFeatureId::kNtQueryLegacy:
        return KernelPageLayoutKind::kTable;
    ...
    case KernelFeatureId::kDriverStatus:
        return KernelPageLayoutKind::kDualTable;
    case KernelFeatureId::kCallbackIntercept:
        return KernelPageLayoutKind::kRuntimePanel;
    default:
        return KernelPageLayoutKind::kTable;
    }
}
...
std::wstring layoutKindText(const KernelPageLayoutKind kind) {
    switch (kind) {
    case KernelPageLayoutKind::kTable: return L"表格";
    case KernelPageLayoutKind::kTree: return L"树表";
    case KernelPageLayoutKind::kTreeWithPropertyTable: return L"树 + 属性表";
    case KernelPageLayoutKind::kTableWithDetail: return L"表格 + 详情";
    case KernelPageLayoutKind::kDualTable: return L"双表分割";
    case KernelPageLayoutKind::kRuntimePanel: return L"运行态控制面板";
    default: return L"表格";
    }
}
...
std::vector<std::wstring> columnAliases(const KernelFeatureId featureId, const std::wstring& columnName) {
    ...
    if (featureId == KernelFeatureId::kAtomTable) {
    ...
    if (featureId == KernelFeatureId::kSsdt || featureId == KernelFeatureId::kShadowSsdt) {
    ...
    if (featureId == KernelFeatureId::kInlineHook) {
        if (columnName == L"模块") return { L"Module" };
        if (columnName == L"函数") return { L"Function" };
    ...
    if (featureId == KernelFeatureId::kIatEatHook) {
    ...
    if (featureId == KernelFeatureId::kDriverStatus) {
    ...
    if (featureId == KernelFeatureId::kCallbackIntercept) {
    ...
    if (featureId == KernelFeatureId::kProcessCrossView) {
    ...
    if (featureId == KernelFeatureId::kThreadCrossView) {
    ...
    if (featureId == KernelFeatureId::kCpuHardwareSnapshot) {
    ...
    if (featureId == KernelFeatureId::kPhysicalMemoryLayout) {
    ...
    if (featureId == KernelFeatureId::kMutationAudit) {
    ...
    if (featureId == KernelFeatureId::kKeyboardHotkeys) {
    ...
    if (featureId == KernelFeatureId::kKeyboardHooks) {
    ...
    if (featureId == KernelFeatureId::kDynDataCapabilities) {
    ...
    if (featureId == KernelFeatureId::kPdbProfileStatus) {
    ...
    if (featureId == KernelFeatureId::kCidTableSummary) {
    ...
    if (featureId == KernelFeatureId::kIpcSummary) {
    ...
    if (featureId == KernelFeatureId::kHookAuditSummary) {
    ...
    if (featureId == KernelFeatureId::kMinifilterBypassPids) {
    ...
    if (featureId == KernelFeatureId::kKernelTimerDpc) {
    ...
    if (featureId == KernelFeatureId::kIoctlRegistry) {
