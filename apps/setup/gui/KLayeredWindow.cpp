...
bool ensureWindowClass() {
    ...
    std::call_once(once, []() {
        ...
        const ATOM kAtom = ::RegisterClassExW(&wc);
        ok = kAtom != 0 || ::GetLastError() == ERROR_CLASS_ALREADY_EXISTS;
    });
    return ok;
}
...
std::wstring utf8ToWide(const std::string& text) {
    ...
    const int kCount = ::MultiByteToWideChar(CP_UTF8, 0, text.c_str(), -1, nullptr, 0);
    if (kCount <= 1) {
        return std::wstring();
    }
    std::wstring wide(static_cast<std::size_t>(kCount - 1), L'\0');
    ::MultiByteToWideChar(CP_UTF8, 0, text.c_str(), -1, &wide[0], kCount);
    return wide;
}
...
bool updateLayeredPixels(HWND hwnd, int screenX, int screenY, int width, int height, const std::vector<unsigned char>& pixels) {
    ...
    const BOOL kOk = ::UpdateLayeredWindow(hwnd, screenDc, &destination, &size, memoryDc, &source, 0, &blend, ULW_ALPHA);
...
bool KLayeredImageWindow::showPngForWindow(Fl_Window* owner, const std::string& pngPathUtf8, int offsetX, int offsetY, int width, int height) {
    ...
    const int kScreenNumber = owner->screen_num();
    float scale = Fl::screen_scale(kScreenNumber);
    if (scale <= 0.0f) {
        scale = 1.0f;
    }

    const int kPixelOffsetX = static_cast<int>(std::lround(static_cast<double>(offsetX) * scale));
    const int kPixelOffsetY = static_cast<int>(std::lround(static_cast<double>(offsetY) * scale));
    const int kPixelWidth = std::max(1, static_cast<int>(std::lround(static_cast<double>(width) * scale)));
    const int kPixelHeight = std::max(1, static_cast<int>(std::lround(static_cast<double>(height) * scale)));
    return showPngAt(pngPathUtf8, ownerRect.left + kPixelOffsetX, ownerRect.top + kPixelOffsetY, kPixelWidth, kPixelHeight, ownerHwnd);
}
...
bool KLayeredImageWindow::ensureWindow(HWND owner) {
    ...
    owner_ = owner;
    const DWORD kExStyle = WS_EX_LAYERED | WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE | (clickThrough_ ? WS_EX_TRANSPARENT : 0);
