...
int showUnsupportedOsDialog(const OsInfo& os, bool chinese) {
    const std::wstring kVersion = L"Windows " + std::to_wstring(os.major) + L"." + std::to_wstring(os.minor) + L" (Build " + std::to_wstring(os.build) + L")";
    std::wstring body = chinese ? L"此系统早于 Windows 10，Ksword 不支持该系统。\n检测到版本：" + kVersion : L"This system is older than Windows 10 and is not supported by Ksword.\nDetected version: " + kVersion;
...
int showEarlyWindowsChoiceDialog(bool chinese) {
    const std::wstring kTitle = text(chinese, L"Ksword 兼容性检查", L"Ksword compatibility check");
    const std::wstring kInstruction = text(chinese, L"当前 Windows 版本早于主程序的 Qt 6.9.3 要求", L"This Windows version predates the main program's Qt 6.9.3 requirement");
    const std::wstring kContent = text(chinese, L"主程序可能无法良好运行，但 Light 版本仍支持早期 Windows 10。请选择启动目标。", L"The main program may not run well, while the Light edition supports early Windows 10. Choose a launch target.");
    const std::wstring kMainButtonValue = text(chinese, L"强制运行主程序", L"Force main program");
    const std::wstring kLightButtonValue = text(chinese, L"运行 Light 版本", L"Run Light version");
    TASKDIALOG_BUTTON buttons[] = {
        { kMainButton, kMainButtonValue.c_str() },
        { kLightButton, kLightButtonValue.c_str() },
    };
    TASKDIALOGCONFIG config = { sizeof(config) };
    config.dwFlags = TDF_ALLOW_DIALOG_CANCELLATION;
    config.pszWindowTitle = kTitle.c_str();
    config.pszMainInstruction = kInstruction.c_str();
    config.pszContent = kContent.c_str();
...
int showMissingDataDialog(bool chinese) {
    const std::wstring kTitle = text(chinese, L"Ksword 兼容性检查", L"Ksword compatibility check");
    const std::wstring kInstruction = text(chinese, L"发现不支持的部分", L"Some system data is not supported");
    const std::wstring kContent = text(chinese,
    ...
    const std::wstring kIgnoreButtonValue = text(chinese, L"忽略", L"Ignore");
    const std::wstring kUploadButtonValue = text(chinese, L"上传", L"Prepare upload");
    TASKDIALOG_BUTTON buttons[] = {
        { kIgnoreButton, kIgnoreButtonValue.c_str() },
        { kUploadButton, kUploadButtonValue.c_str() },
    };
    TASKDIALOGCONFIG config = { sizeof(config) };
    config.dwFlags = TDF_ALLOW_DIALOG_CANCELLATION;
    config.pszWindowTitle = kTitle.c_str();
...
int showUploadElevationFailureDialog(bool chinese) {
    const std::wstring kTitle = text(chinese, L"需要管理员权限", L"Administrator permission required");
    const std::wstring kInstruction = text(chinese, L"采集数据需要管理员权限", L"Administrator permission is required to collect data");
    const std::wstring kContent = text(chinese, L"Windows 未接受提权请求。可以重试，或取消上传并继续启动。", L"Windows did not accept the elevation request. Retry, or cancel upload and continue launching.");
    const std::wstring kRetryButtonValue = text(chinese, L"重试", L"Retry");
    const std::wstring kCancelButtonValue = text(chinese, L"取消上传", L"Cancel upload");
    TASKDIALOG_BUTTON buttons[] = {
        { kRetryButton, kRetryButtonValue.c_str() },
        { kCancelButton, kCancelButtonValue.c_str() },
    };
    TASKDIALOGCONFIG config = { sizeof(config) };
    config.dwFlags = TDF_ALLOW_DIALOG_CANCELLATION;
    config.pszWindowTitle = kTitle.c_str();
...
void showCollectionProgress(bool chinese, CollectionProgress* progress) {
    ...
    *progress = {};
    ...
    HWND window = CreateWindowExW(WS_EX_DLGMODALFRAME, kClassName,
        ...
        WS_CAPTION | WS_SYSMENU, CW_USEDEFAULT, CW_USEDEFAULT, kWidth, kHeight,
    ...
    RECT work = {}; SystemParametersInfoW(SPI_GETWORKAREA, 0, &work, 0);
    SetWindowPos(window, HWND_TOP, work.left + (work.right - work.left - kWidth) / 2,
        work.top + (work.bottom - work.top - kHeight) / 2, kWidth, kHeight, SWP_SHOWWINDOW);
    UpdateWindow(window);
...
void updateCollectionProgress(CollectionProgress* progress, int percent, const std::wstring& text) {
    ...
    const int kPosition = percent < 0 ? 0 : (percent > 100 ? 100 : percent);
    if (progress->bar) SendMessageW(progress->bar, PBM_SETPOS, static_cast<WPARAM>(kPosition), 0);
