...
namespace
{
        ...
        QStringList* warningListOut)
    {
        ...
        static const QSet<QString> kSupportedKeys{
        ...
        {
            if (!kSupportedKeys.contains(iterator.key()) && warningListOut != nullptr)
            {
...
quint32 computeCallbackCrc32(const QByteArray& rawBytes)
{
    ...
    for (const unsigned char kValueByte : rawBytes)
    {
        crcValue ^= static_cast<quint32>(kValueByte);
    ...
    QString* errorTextOut)
{
    ...
    static const QSet<QString> kRootKeys{
    ...
    for (auto iterator = kRootObject.begin(); iterator != kRootObject.end(); ++iterator)
    {
        if (!kRootKeys.contains(iterator.key()) && warningListOut != nullptr)
        {
    ...
    importedDocument.schemaVersion = static_cast<quint32>(
        kRootObject.value(QStringLiteral("schemaVersion"))
    ...
    importedDocument.exportedAtUtc = kRootObject.value(QStringLiteral("exportedAtUtc")).toString();
    importedDocument.appVersion = kRootObject.value(QStringLiteral("appVersion")).toString();
    ...
    const QJsonObject kGlobalSettingsObject = kRootObject.value(QStringLiteral("globalSettings")).toObject();
    importedDocument.globalEnabled = kGlobalSettingsObject.value(QStringLiteral("globalEnabled")).toBool(true);
    importedDocument.ruleVersion = static_cast<quint64>(kGlobalSettingsObject.value(QStringLiteral("ruleVersion")).toInteger(1));

    if (!kRootObject.value(QStringLiteral("groups")).isArray() ||
    ...
    for (const QJsonValue& groupValue : kRootObject.value(QStringLiteral("groups")).toArray())
    {
    ...
    for (const QJsonValue& ruleValue : kRootObject.value(QStringLiteral("rules")).toArray())
    {
    ...
    importedDocument.crc32 = static_cast<quint32>(kRootObject.value(QStringLiteral("crc32")).toInteger(0));
    if (importedDocument.crc32 != 0U)
    {
        QJsonObject rootWithoutCrc = kRootObject;
        rootWithoutCrc.remove(QStringLiteral("crc32"));
        const quint32 kCalculatedCrc = computeCallbackCrc32(QJsonDocument(rootWithoutCrc).toJson(QJsonDocument::Indented));
        if (kCalculatedCrc != importedDocument.crc32)
        {
            if (errorTextOut != nullptr)
            {
    ...
    QString* errorTextOut)
{
    ...
    if (!kValidationResult.success)
    {
        if (errorTextOut != nullptr)
        {
            *errorTextOut = kernelText("kernel.callback.rules.compile.validation_failed", QStringLiteral("编译失败：%1"))
                .arg(kValidationResult.errorList.join(QStringLiteral("；")));
        }
        return false;
    }
    ...
    std::sort(sortedRules.begin(), sortedRules.end(), [&groupPriorityById](const CallbackRuleModel& leftRule, const CallbackRuleModel& rightRule) {
        const qint32 kLeftGroupPriority = groupPriorityById.value(leftRule.groupId, 0);
        const qint32 kRightGroupPriority = groupPriorityById.value(rightRule.groupId, 0);
        if (kLeftGroupPriority != kRightGroupPriority)
        {
            return kLeftGroupPriority < kRightGroupPriority;
        }
    ...
    const quint32 kHeaderSize = static_cast<quint32>(sizeof(KSWORD_ARK_CALLBACK_RULE_BLOB_HEADER));
    const quint32 kGroupBytes = static_cast<quint32>(groupBlobList.size() * sizeof(KSWORD_ARK_CALLBACK_GROUP_BLOB));
    const quint32 kRuleBytes = static_cast<quint32>(ruleBlobList.size() * sizeof(KSWORD_ARK_CALLBACK_RULE_BLOB));
    const quint32 kStringBytes = static_cast<quint32>(stringPool.size());

    KSWORD_ARK_CALLBACK_RULE_BLOB_HEADER header{};
    header.size = kHeaderSize + kGroupBytes + kRuleBytes + kStringBytes;
    ...
    header.groupOffsetBytes = kHeaderSize;
    header.ruleOffsetBytes = header.groupOffsetBytes + kGroupBytes;
    header.stringOffsetBytes = header.ruleOffsetBytes + kRuleBytes;
    header.stringBytes = kStringBytes;
    header.crc32 = 0;
    ...
    {
        blobBytes.append(reinterpret_cast<const char*>(groupBlobList.constData()), kGroupBytes);
    }
    if (!ruleBlobList.isEmpty())
    {
        blobBytes.append(reinterpret_cast<const char*>(ruleBlobList.constData()), kRuleBytes);
    }
    ...
    const quint32 kCrc32Value = computeCallbackCrc32(blobBytes);
    auto* writableHeader = reinterpret_cast<KSWORD_ARK_CALLBACK_RULE_BLOB_HEADER*>(blobBytes.data());
    writableHeader->crc32 = kCrc32Value;
