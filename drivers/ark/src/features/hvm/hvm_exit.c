...
/*
 * This is the current EPT hierarchy for this processor. The EPTP switch backend modifies only this specific
 * field in the exit path, and **only modifies this VMCS instance**. The hierarchies of other processors remain
 * unaffected, which is precisely why the window does not need to be compressed into a single instruction.
 */
#define KSW_VMCS_EPT_POINTER 0x201AUL
/* Guest CR3; R-1 process handling relies on it to identify the running address space. */
...
kswordArkHvmExitPublishTelemetry(
    ...
    _In_ const KswHvmVmexitTelemetry* telemetry,
...
kswordArkHvmExitPublishWatchHit(
    ...
    _In_ ULONGLONG guestLinearAddress,
    _In_ ULONG access,
    _In_ const KswHvmEptWatchHit* watchHit,
    ...
    )
{
    ...
    const ULONGLONG kExits = context->costExits;
    ...
    }
    {
        ...
        static const ULONG kNames[6] = { 23UL, 25UL, 48UL, 30UL, 12UL, 0xFFFFUL };
        ULONG bucket = 0UL;

        for (bucket = 0UL; bucket < 6UL; ++bucket) {
            const ULONGLONG kHits = context->costReasonCount[bucket];
            ...
            row.exitReason = kNames[bucket];
            row.qualification = context->costReasonCycles[bucket] / kHits;
            row.guestPhysicalAddress = kHits;
    ...
    }
    {
        ...
        for (reason = 0UL; reason < 64UL; ++reason) {
            const ULONGLONG kHits = context->nested.l2ExitReasonCounts[reason];
    ...
    }
    {
        ...
        for (number = 0UL; number < 5UL; ++number) {
            ...
            for (access = 0UL; access < 4UL; ++access) {
                const ULONGLONG kHits = context->nested.l2CrCounts[number][access];
    ...
    row.qualification = context->costTotalCycles / kExits;
    row.guestPhysicalAddress = context->costTelemetryCycles / kExits;
    row.guestLinearAddress = context->costNestedCycles / kExits;
    row.guestRip = context->costReflectCycles / kExits;
    /* The VMCS reads every exit begins with, and the EPT work some end with. */
    row.exitReason = (ULONG)(context->costVmcsReadCycles / kExits);
    row.status = (LONG)(context->costEptCycles / kExits);
    row.access = (ULONG)context->apicId;
...
{
    const ULONGLONG kCostStart = __rdtsc();
    const ULONG kAction = kswordArkHvmResidentVmExitDispatchBody(frame, context);

    if (context != NULL) {
        const ULONGLONG kSpent = __rdtsc() - kCostStart;
        const ULONG kBucket = (context->costLastBucket < 6UL)
            ? context->costLastBucket
            : 5UL;

        context->costTotalCycles += kSpent;
        context->costReasonCycles[kBucket] += kSpent;
        context->costReasonCount[kBucket] += 1ULL;
...
{
    KswHvmVmexitTelemetry telemetry = { 0 };
    ...
    {
        const ULONGLONG kReadStart = __rdtsc();
        ...
        context->costVmcsReadCycles += (__rdtsc() - kReadStart);
    }
     ...
     */
    {
        const ULONGLONG kEmptyStart = __rdtsc();

        context->costEptCycles += (__rdtsc() - kEmptyStart);
    }
    ...
    nestedBasicReason =
        telemetry.reason & KSW_HVM_VMEXIT_REASON_BASIC_MASK;
    if (context->nested.inL2) {
        const ULONGLONG kReflectStart = __rdtsc();
        const ULONG kRoute = kswordArkHvmNestedL2Reflect(
            context,
            frame,
            nestedBasicReason);

        context->costReflectCycles += (__rdtsc() - kReflectStart);
         ...
         */
        if (kRoute != KSW_HVM_L2_ROUTE_NOT_L2 &&
            kRoute != KSW_HVM_L2_ROUTE_SERVICE_LOCALLY) {
    ...
    /* Hand every other VMCALL to whatever hypervisor sits above us. */
         ...
         * 28.3.3.1 around revision 070. The
    ...
    /* Resolve MSR access the bitmap deliberately let exit. */
         ...
         */
        if (kswordArkHvmExitFilterVmxCapabilityMsr(frame, kIsWrite)) {
             ...
             */
            const ULONG kPolicyResult = kswordArkHvmMsrPolicyApply(
                context->runtime,
                frame,
                kIsWrite);

            if (kPolicyResult == KSW_HVM_MSR_POLICY_RESULT_HANDLED) {
                /* Continue at the instruction following the MSR access. */
                handled = kswordArkHvmExitAdvanceRip(
                    telemetry.instructionLength);
            } else if (kPolicyResult ==
    ...
    /* Apply control-register policy to a masked bit or a tracked CR3 load. */
        ...
        handled = kswordArkHvmCrPolicyHandleControlRegister(
            ...
            telemetry.qualification);
        /*
         * When the address space changes, switch the layer accordingly.
         *
         * The scope of R-1 process disposition relies entirely on this step: matching address spaces enter the
         * restricted layer, while all others revert to the base. This is placed here rather than checking CR3
         * page-by-page because the page-by-page path requires a "deny-then-allow" flip, which necessitates
         * monitor-trap—a bit unavailable on nested targets, making that path impossible to run there.
         *
         * Only touch the VMCS when the table is non-empty. When the table is empty, this entire block is
         * just a comparison, and it is traversed on every address space switch during the resident period.
         */
        if (handled &&
            ...
            if (kswordArkHvmVmcsFieldLoad(
                ...
                if (!kswordArkHvmProcessSelectHierarchy(
                        ...
                        &targetEptp)) {
                    /*
                     * When returning to the base, use the set of EPT pointers that the processor
                     * **actually** has loaded: if a private hierarchy is enabled, the shared pointer is not
                     * what it is using; writing it back would switch this core to someone else's hierarchy.
                     */
                ...
                /* If hierarchy cannot be calculated, do not write: failure maintains the current state; never switch to ring 0. */
                if (targetEptp != 0ULL) {
    ...
    /* Apply one bounded EPT violation rule. */
        ...
        access = kswordArkHvmExitDecodeEptAccess(
            telemetry.qualification);
        /*
         * R-1 process disposition is claimed before view and rule checks.
         *
         * Order is not preferred: the page being handled has no execute bit in its own restricted hierarchy, while
         * the view and rules are mounted on the base. The same guest physical address might be targeted by both the
         * handler and a rule; in this case, executing the rule first would allow the instruction fetch according to
         * the rule's semantics, causing the handler to fail silently—another case of 'mounted but nothing happened'.
         *
         * Reversing this does not harm views or rules: this module only recognizes violations and only
         * pages listed in its own table; all others return None to fall back to the original path.
         */
        {
            KswHvmProcessAction disposition =
                kswordArkHvmProcessHandleViolation(
                    context->runtime,
                    guestPhysicalAddress,
                    access);

            if (disposition != kKswHvmProcessActionNone) {
                if (disposition == kKswHvmProcessActionResume) {
                    /*
                     * This entry has been disabled, yet the processor is still spinning in a restricted hierarchy. Restore
                     * the pointer to the set it **truly** should use and continue in place without injecting anything.
                     *
                     * This is the only place where the 'frozen core' is unfrozen: the unfreezing
                     * modifies only a single field. The spinning core receives no notification for
                     * this change; it reaches this point on its next violation and unfreezes itself.
                     */
                    ULONGLONG baseEptp =
                    ...
                    handled = baseEptp != 0ULL &&
                        kswordArkHvmVmcsFieldStore(
                            KSW_VMCS_EPT_POINTER,
                            (SIZE_T)baseEptp) == 0U;
                } else if (disposition == kKswHvmProcessActionTerminate) {
                    /*
                     * Hardware exception restarts the faulting instruction, so RIP remains unchanged and the instruction length
                     * is zero. Termination relies on the guest converting this unhandled exception into a process termination.
                     */
                    handled = kswordArkHvmExitInjectUndefinedOpcode();
                } else {
                    /*
                     * Restart the faulting instruction: freezing requires that 'this instruction never retires', so the process
                     * state remains unchanged by a single byte; upon resumption, execution continues from the same point.
                     */
                    handled = kswordArkHvmExitInjectPageFault(
                        guestLinearAddress,
                        access);
                }
                /* Log the event and finalize based on the injection result; do not proceed to the view or rules. */
                kswordArkHvmExitPublishTelemetry(
                    context,
                    &telemetry,
                    guestPhysicalAddress,
                    ...
                    handled
                        ? STATUS_SUCCESS
                        : STATUS_HV_OPERATION_FAILED);
                /* Complete the termination of this exit dispatch. */
                return handled
        ...
        if (handled && viewSwitch.requested) {
            ...
            /*
             * The execution view has already been selected; only then does R-1 injection decide whether to change RIP.
             *
             * Order is strict: RIP points to the shellcode in the shadow page, and the shadow page only contains this content
             * after switching to the execution level. If RIP is modified before the switch, the guest will fetch instructions
             * from the same offset in the real page during that instant—where the original bytes reside, not the shellcode.
             */
            if (handled) {
                SIZE_T injectCr3 = 0U;
                SIZE_T injectRip = 0U;
                ...
                if (kswordArkHvmVmcsFieldLoad(
                    ...
                    kswordArkHvmInjectHijackRip(
                        ...
                        &hijackRip)) {
                    /*
                     * If writing fails, maintain the original RIP. It is acceptable for
                     * the payload not to run, but changing RIP to a partial value is not.
                     */
    ...
    /* Dispatch bounded VMX instruction semantics without claiming L2 active. */
        ...
        basicReason)) {
        const ULONGLONG kNestedStart = __rdtsc();
        ...
        handled = kswordArkHvmNestedHandleExit(
            ...
            basicReason,
            telemetry.instructionLength);
        context->costNestedCycles += (__rdtsc() - kNestedStart);
