...
// Purpose:
// 1) enumerate SymbolicLink objects in common Object Manager directories from R3;
// 2) For each SymbolicLink, parse the target path and attempt to map DOS path candidates;
// 3) Retain lines for directory open failures and link resolution failures to facilitate debugging of permission/Session differences.
// ============================================================
...
namespace
{
    ...
    {
        handleOut = nullptr;
        const std::wstring kPathText = normalizeObjectPath(directoryPathText).toStdWString();
        UNICODE_STRING objectName{};
        initializeUnicodeString(objectName, kPathText);
    ...
    {
        handleOut = nullptr;
        const std::wstring kPathText = normalizeObjectPath(symbolicLinkPathText).toStdWString();
        UNICODE_STRING objectName{};
        initializeUnicodeString(objectName, kPathText);
        ...
        QString& statusTextOut)
    {
        ...
        const NTSTATUS kOpenStatus = openSymbolicLinkObject(api, symbolicLinkPathText, symbolicLinkHandle.handle);
        if (!NT_SUCCESS(kOpenStatus))
        {
            statusTextOut = QStringLiteral("NtOpenSymbolicLinkObject: %1")
                .arg(ntStatusToText(api.ntdllModule, kOpenStatus));
            return false;
        }
        ...
        for (int retry = 0; retry < 6; ++retry)
        {
            ...
            if (NT_SUCCESS(kQueryStatus))
            {
                ...
                statusTextOut = QStringLiteral("OK: %1").arg(ntStatusToText(api.ntdllModule, kQueryStatus));
                return true;
            }
            if (!isNeedGrowStatus(kQueryStatus))
            {
    ...
    {
        const std::vector<QString> kCandidates = queryKernelSymbolicLinkDosPathCandidates(ntPathText);
        if (kCandidates.empty())
        {
            return QString();
        }
        QStringList candidateTextList;
        for (const QString& candidateText : kCandidates)
        {
    ...
    QString& errorTextOut)
{
    ...
    const QStringList kSourceDirectories = buildSourceDirectories(api);
    std::vector<KernelSymbolicLinkEntry> rows;
    rows.reserve(512);

    for (const QString& directoryPath : kSourceDirectories)
    {
    ...
    std::sort(rows.begin(), rows.end(), [](const KernelSymbolicLinkEntry& left, const KernelSymbolicLinkEntry& right) {
        const int kDirectoryCompare = QString::compare(left.sourceDirectory, right.sourceDirectory, Qt::CaseInsensitive);
...
std::vector<QString> queryKernelSymbolicLinkDosPathCandidates(const QString& ntPathText)
{
    ...
    const QString kNormalizedNtPath = normalizeObjectPath(ntPathText);
    ...
    for (wchar_t driveLetter = L'A'; driveLetter <= L'Z'; ++driveLetter)
    {
        ...
        while (*cursor != L'\0')
        {
            const QString kMappingText = QString::fromWCharArray(cursor).trimmed();
            if (!kMappingText.isEmpty()
                && kNormalizedNtPath.startsWith(kMappingText, Qt::CaseInsensitive))
            {
                const QString kSuffixText = kNormalizedNtPath.mid(kMappingText.size());
                QString candidateText = QString::fromWCharArray(driveName);
                candidateText += kSuffixText.isEmpty() ? QStringLiteral("\\") : kSuffixText;
