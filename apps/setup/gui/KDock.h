...
public:
    ...
    bool floatingGeometry(int& rootX, int& rootY, int& width, int& height) const;
...
private:
    ...
    KDockArea* area_;
    Fl_Window* floatingWindow_;
    bool pinned_;
    bool dragging_;
    bool dragStarted_;
    bool layoutingContent_;
    int dragStartRootX_;
    int dragStartRootY_;
    int dragOffsetX_;
    int dragOffsetY_;
...
private:
    ...
    int tabIndexAt(int localX, int localY) const;
...
private:
    ...
    int hoverIndex_;
    bool defaultArea_;
    bool layoutingDocks_;
    bool tabDragging_;
    int tabDragIndex_;
    int tabDragStartRootX_;
    int tabDragStartRootY_;
    KDockWidget* tabDragWidget_;
};
...
public:
    ...
    void dockWidgetFromDrag(KDockWidget* widget, int rootX, int rootY);

    // Updates overlay state while a dock title bar is being dragged.
    void updateDockDrag(KDockWidget* widget, int rootX, int rootY);
...
private:
    ...
    KDockAreaPosition dropPositionForRootPoint(int rootX, int rootY) const;

    // Computes the precise overlay/drop target for one root-screen point.
    KDockDropTarget dropTargetForRootPoint(KDockWidget* widget, int rootX, int rootY) const;

    // Computes a tolerant drop target from the dragged dock's floating window.
    KDockDropTarget dropTargetForFloatingWidget(KDockWidget* widget, int rootX, int rootY) const;
    ...
    void layoutNode(KDockLayoutNode* node, int nodeX, int nodeY, int nodeW, int nodeH);
...
private:
    ...
    KDockArea* bottom_;
    KDockArea* center_;
    KDockLayoutNode* rootNode_;
    std::vector<KDockArea*> allAreas_;
    std::vector<KDockWidget*> knownDocks_;
    KDockDropTarget activeDropTarget_;
    KDockWidget* draggingWidget_;
    int nextAreaId_;
    int leftWidth_;
    int rightWidth_;
    int topHeight_;
    int bottomHeight_;
    bool layoutingAreas_;
};
