#include "NetworkDownloadTools.h"
...
namespace ks::network
{
        ...
        const int maxThreadCount)
    {
        ...
        const int kSafeMaxThreadCount = std::max(1, maxThreadCount);
        const int kByteBoundedThreadCount = static_cast<int>(std::min<std::uint64_t>(
            static_cast<std::uint64_t>(kSafeMaxThreadCount),
            totalBytes));
        actualThreadCount = std::max(1, std::min(actualThreadCount, kByteBoundedThreadCount));
        ...
        const std::uint64_t kAverageSegmentBytes = totalBytes / static_cast<std::uint64_t>(actualThreadCount);
        const std::uint64_t kRemainSegmentBytes = totalBytes % static_cast<std::uint64_t>(actualThreadCount);
        std::uint64_t beginByte = 0;
        for (int index = 0; index < actualThreadCount; ++index)
        {
            const std::uint64_t kSegmentBytes = kAverageSegmentBytes +
                (index < static_cast<int>(kRemainSegmentBytes) ? 1ULL : 0ULL);
            const std::uint64_t kEndByte = beginByte + kSegmentBytes - 1ULL;
            plan.segments.push_back(DownloadSegmentPlan{ index, beginByte, kEndByte, kSegmentBytes });
            beginByte = kEndByte + 1ULL;
        }
        return plan;
    }
        ...
        const std::uint64_t totalBytes)
    {
        ...
        const std::uint64_t kClampedDownloaded = std::min(downloadedBytes, totalBytes);
        return std::clamp(
            static_cast<double>(kClampedDownloaded) / static_cast<double>(totalBytes),
            0.0,
            1.0);
    }
        ...
        const std::uint64_t elapsedMs)
    {
        ...
        // Use a floating-point intermediate value to prevent transferredBytes * 1000 from overflowing during long tasks or with large files.
        const long double kRateValue =
            (static_cast<long double>(transferredBytes) * 1000.0L) /
            static_cast<long double>(elapsedMs);
        if (kRateValue >= static_cast<long double>(std::numeric_limits<std::uint64_t>::max()))
        {
            return std::numeric_limits<std::uint64_t>::max();
        }
        return static_cast<std::uint64_t>(kRateValue);
    }
} // namespace ks::network
